...
different combinations. The lock has three dials, each numbered 1..N (1 <= N <= 100), where 1 and N are adjacent since the dials 
are circular. There are two combinations that open the lock, one set by Farmer John, and also a "master" combination set by the 
lock maker.

The lock has a small tolerance for error, however, so it will open even if the numbers on the dials are each within at most 2 
positions of a valid combination.

For example, if Farmer John's combination is (1,2,3) and the master combination is (4,5,6), the lock will open if its dials are 
set to (1,3,5) (since this is close enough to Farmer John's combination) or to (2,4,8) (since this is close enough to the master 
combination). Note that (1,5,6) would not open the lock, since it is not close enough to any one single combination.

Given Farmer John's combination and the master combination, please determine the number of distinct settings for the dials that 
...
public class combo {
	...
	public static void main(String[] args) {
		try(BufferedReader f = new BufferedReader(new FileReader("combo.in"));
		...
		while(f.ready()){
			...
			for(int i = 0; i < 3; i++){
				master[i] = Integer.parseInt(dt.nextToken());
			}
			out.println(distinctDials(max, john, master));
		}
		} catch (IOException e) {}
	}
	
	public static int distinctDials(int max, int[] john, int[] master){
		if(max < 6){	//if 1 or 2 or 3 or 4 or 5, anything works
			return max * max * max;
		}
		if(max < 10){	//up to max=9, there are situations where you have overlaps in both directions. Ex. 3, 7 has overlaps at 1,2, AND 5. I did not want to deal with that confusion
			return bruteForce(max, john, master);
		}
		int[] overlap = new int[3];
		boolean atLeastOne = false;
		for(int i = 0; i < 3; i++){
			int mid = Math.abs(john[i] - master[i]);
			if(mid > max/2){	//shortest path is going the other way
				mid = max - mid;
			}
			if(mid > 4){
				overlap[i] = 1;
		...
		if(!atLeastOne){	//no overlaps
			return 250;
		}
		return 250 - overlap[0] * overlap[2] * overlap[1];
	}

	public static int bruteForce(int max, int[] john, int[] master){
		int valid = 0;
		for(int i = 1; i <= max; i++){
			for(int j = 1; j <= max; j++){
				for(int k = 1; k <= max; k++){
					int[] temp = {i, j, k};
					if(isValid(temp, john, max) || isValid(temp, master, max)){	//matches with one of them
						valid++;
					}
				}
			}
	...
	public static boolean isValid(int[] attempt, int[] key, int max){
		for(int i = 0; i < 3; i++){
			int dist = Math.abs(key[i] - attempt[i]);
			if(dist > max/2){
				dist = max - dist;	//shorter route
			}
			if(dist > 2){
				return false;
			}
